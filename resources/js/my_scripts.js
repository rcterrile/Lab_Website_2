...
function loadStatsPage() {
	...
	for(var i = 2; i < 60; i += 5) {
		var sc1 = elements[i].innerHTML;
		var sc2 = elements[i+1].innerHTML;
		if (sc1 > sc2){
			elements[i+2].innerHTML = "Win";
			wins1++;
		}
		else {
			elements[i+2].innerHTML = "Loss";
			losses1++;
		}
	}
	elements[60].innerHTML = wins1;
	elements[61].innerHTML = losses1;
}
/*
...
function loadPlayersPage() {
	var divs = document.getElementById("player_selector");
	var tm = 0;
	for (var i = 0; i < 4; i++) {
		var tag = document.createElement('a');
		tag.setAttribute('href',"#");
		tag.classList.add('dropdown-item');
		tag.innerText = players[i].name;
		tm = i;
		tag.setAttribute('onclick', "switchPlayers(" + tm + ")");
		divs.appendChild(tag);
	}
	var button = document.getElementById("selectPlayerButton");
	//button.addEventListener('click', ...);
	//button.setAttribute('onclick', "switchPlayers(button.selectedIndex)")";
	console.log(button.selectedIndex);
}
function switchPlayers(playerNum) {
	console.log("give: " + playerNum);
